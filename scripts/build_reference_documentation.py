...
MODULE_SUMMARIES = {
    ...
    "scripts/build_reference_documentation.py": "Generator for the function and test references.",
}
...
def _function_signature(node: ast.FunctionDef | ast.AsyncFunctionDef) -> str:
    ...
    header = header.removeprefix("async def ").removeprefix("def ")
    return header.removeprefix(node.name)
...
def _walk_symbols(
    ...
    for node in body:
        if isinstance(node, ast.ClassDef):
            ...
            symbols.append(
                Symbol(relative, name, "class", "", node.lineno, ast.get_docstring(node, clean=True), ())
            )
            symbols.extend(_walk_symbols(file, root, node.body, name + ".", "class"))
            ...
            symbols.extend(_walk_symbols(file, root, node.body, name + ".", "function"))
    return symbols
...
def collect_symbols(root: Path) -> list[Symbol]:
    ...
    for file in sorted(root.rglob("*.py")):
        ...
        try:
            ...
            continue
        symbols.extend(_walk_symbols(file, root, tree.body))
    return symbols
...
def describe_symbol(symbol: Symbol) -> str:
    ...
    if symbol.kind == "class":
        return f"Data model for {subject}."
    return f"Implementation helper for {subject}."


def _module_summary(path: str) -> str:
...
def render_function_reference(symbols: list[Symbol]) -> str:
    ...
    for file, items in grouped.items():
        ...
        for symbol in items:
            ...
            purpose = _escape_table_cell(describe_symbol(symbol))
            lines.append(
...
def _relevant_calls(symbol: Symbol) -> str:
    ignored_prefixes = (
        "assert",
        "pd.",
        "json.",
...
def render_test_reference(symbols: list[Symbol]) -> str:
    ...
    for file, items in grouped.items():
        ...
        if tests:
            ...
            for symbol in tests:
                purpose = _escape_table_cell(describe_symbol(symbol))
                calls = _escape_table_cell(_relevant_calls(symbol))
                lines.append(
...
def main() -> int:
    ...
    stale = [
        ...
        if not _write_or_check(path, content, args.check)
    ]
    for path in stale:

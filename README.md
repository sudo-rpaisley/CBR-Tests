...
The selector groups run options by purpose, explains the highlighted field in a footer, shows the detected maximum beside worker selection, and lets you choose common run flags, browse for the dataset, set paths, display mode, worker count, taxonomy controls, and field-translation report options before starting the run. During a TUI-launched run, the interactive dashboard supports branch selection with ↑/↓, expand/collapse with Enter/Space, and metric-list scrolling inside expanded branches with PgUp/PgDn or u/d. If terminal keyboard capture is unavailable, the dashboard says so instead of pretending controls are active. After `--tui` or direct `--display interactive` runs, an expandable results screen lets you review human-readable metric results, summary/failure sections, return to the setup menu, quit the program, or run metrics after a dry run. If a dry run finds missing field mappings, the confirmation screen can open a mapping UI with per-field dropdowns of remaining dataset columns.

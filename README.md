...
The selector groups run options by purpose, explains the highlighted field in a footer, shows the detected maximum beside worker selection, and lets you choose common run flags, browse for the dataset, set paths, display mode, worker count, taxonomy controls, and field-translation report options before starting the run. After a TUI-launched run, an expandable results screen lets you review summary/failure sections, return to the setup menu, quit the program, or run metrics after a dry run.

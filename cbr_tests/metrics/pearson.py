...
def validate_candidate_fields(
    ...
    """Validate and coerce fields that may be used for correlation metrics."""
    column_validation: list[dict] = []
...
def compute_pearson_profile(df: pd.DataFrame, runnable_fields: list[str]) -> dict:
    """Compute the Pearson correlation profile for runnable fields."""
    correlation = df[runnable_fields].corr(method="pearson")
    pairs = []

...
def _discover_batch_manifests(root: Path, directory: Path | None = None) -> list[tuple[Path, dict[str, Any]]]:
    """Return valid saved batch manifests without entering generated per-job plan folders."""
...
def _batch_picker(stdscr, root: Path, *, initial_dir: str = "plans") -> list[str] | None:
    """Select one or more already-built batch matrices for a campaign queue."""

    root = root.expanduser().resolve()
    search_root = (root / initial_dir).expanduser().resolve()
    ...
    while True:
        ...
        _safe_addstr(
            ...
            2,
            0,
            f"Found {len(manifests)} batch(es) under {display(search_root)}; selected {len(checked)}",
        )
        if message:
        ...
        for row, (manifest_path, batch) in enumerate(manifests[scroll : scroll + visible], start=start_row):
            ...
            _safe_addstr(
                ...
                row,
                0,
                f"{marker} [{tick}] {name} — {len(batch['jobs'])} jobs — {display(manifest_path)}",
                attr,
            )
        ...
        if not manifests:
            _safe_addstr(
                ...
                start_row,
                0,
                "No valid saved batch manifests were found. Build the batches first, then return here.",
            )
        ...
        if key in (ord("a"), ord("A")):
            checked = {path for path, _ in manifests}
            continue
        ...
        if key in (ord("r"), ord("R")):
            manifests = _discover_batch_manifests(root, search_root)
            checked.intersection_update(path for path, _ in manifests)
            selected = min(selected, max(0, len(manifests) - 1))
            continue
        if key in (10, 13):
            if not checked:
                message = "Select at least one batch with Space (or press a to select all)."
                continue
            ordered = [path for path, _ in manifests if path in checked]
            return [display(path) for path in ordered]

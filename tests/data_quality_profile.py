...
def compute_missing_value_ratio(df: pd.DataFrame, metric: dict) -> dict:
    ...
    for field in fields:
        ...
        field_results.append({
            ...
            "missing_value_ratio": round(field_missing_count / row_count, 6) if row_count else 0.0,
        })
    ...
    return {
        ...
        "summary": {
            ...
            "missing_value_ratio": round(missing_cells / total_cells, 6) if total_cells else 0.0,
        },
    }
...
def compute_duplicate_row_ratio(df: pd.DataFrame, metric: dict) -> dict:
    ...
    if not subset_fields:
        ...
        duplicate_group_count = int(df.loc[df.duplicated(subset=subset_fields, keep=False), subset_fields].drop_duplicates().shape[0])

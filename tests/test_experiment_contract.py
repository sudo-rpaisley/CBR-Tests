...
def test_packet_capture_formats_share_plan_compatibility_family(
    tmp_path: Path,
    declared_format: str,
    dataset_name: str,
):
    plan = _plan()
    plan["applicability"]["dataset_formats"] = [declared_format]

    validate_dataset_format_applicability(plan, tmp_path / dataset_name)

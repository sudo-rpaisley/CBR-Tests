...
| `test_campaign_preflight_checks_every_matrix_before_outputs_start(tmp_path)` (L81) | Verifies that campaign preflight checks every matrix before outputs start. | `_write_batch`, `build_campaign`, `second.write_text`, `_preflight_campaign`, `pytest.raises` |
| `test_campaign_batch_command_passes_resume_retry_and_experiment_controls(tmp_path)` (L97) | Verifies that campaign batch command passes resume retry and experiment controls. | `argparse.Namespace`, `_build_batch_command`, `command.index` |
| `test_toolbox_exposes_campaign_builder_and_runner()` (L125) | Verifies that toolbox exposes campaign builder and runner. | `keys.index`, `_menu_items` |

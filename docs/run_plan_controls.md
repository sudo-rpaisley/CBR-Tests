...
`python run_plan.py --tui` opens a pre-run selector for users who do not want to memorize flags. The selector runs only in an interactive terminal and supports arrow-key navigation, Enter to act on the selected field, Space to toggle boolean flags, `r` to run, and `q` to cancel. A footer explains the highlighted field and the available action, and the worker field shows the detected maximum worker count. It can set the same options exposed by the common CLI flags, including case/plan, dataset selection through a file explorer, output, case ID, display mode, worker count, taxonomy options, sidecar behavior, dry-run behavior, and report output paths. During execution, the interactive display lets users select branches with ↑/↓, expand/collapse metric rows with Enter/Space, and scroll long expanded metric lists with PgUp/PgDn or u/d. If terminal keyboard capture is unavailable, the dashboard shows a warning controls row. After completion, `--tui` and direct `--display interactive` runs return to an expandable results screen instead of exiting immediately; users can scroll with arrow keys, expand/collapse sections with Enter, review human-readable metric results/success/failure dropdowns, go back to the setup menu, quit, or run metrics after a dry run. Dry runs with issues require confirmation before continuing into a real run; from that confirmation screen, `m` opens a field-mapping UI where each missing canonical field gets a dropdown of unused dataset columns. The report options are optional field-translation validation outputs: JSON for automation, text for readable logs, and Markdown for review notes.

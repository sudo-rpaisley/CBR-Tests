...
`python run_plan.py --tui` opens a pre-run selector for users who do not want to memorize flags. The selector runs only in an interactive terminal and supports arrow-key navigation, Enter to act on the selected field, Space to toggle boolean flags, `r` to run, and `q` to cancel. A footer explains the highlighted field and the available action, and the worker field shows the detected maximum worker count. It can set the same options exposed by the common CLI flags, including case/plan, dataset selection through a file explorer, output, case ID, display mode, worker count, taxonomy options, sidecar behavior, dry-run behavior, and report output paths. After completion, it returns to a results screen instead of exiting immediately; users can go back to the setup menu, quit, or run metrics after a dry run. Dry runs with issues require confirmation before continuing into a real run.
